...
from prompts_collection import summary_generator, blog_generator, tweet_generator, newsletter_generator, video_script_generator
import asyncio
...
def generate_content(openai_api_key, text, content ,thread:bool=False):
    ...
    elif content == "Video Script":
        prompt = video_script_generator()
...
def main():
    ...
    video_script = st.sidebar.checkbox("Video Script")
    ...
    if openai_api_key and yt_url:
        if st.button("Submit"):
            ...
            if video_script:
                video_answer = generate_content(openai_api_key=openai_api_key, text=transcript, content="Video Script")

            col1, col2 = st.columns(2)
            with col1:
                if video_script:
                    st.subheader("5 Minute Video Script:")
